...
exports.sendTaskNotifications = onSchedule({
  ...
  const currentHour = parseInt(ukOffset);
  ...
  for (const userDoc of usersSnap.docs) {
    ...
    try {
      ...
      const preferredHour = userData.notifHour !== undefined ? userData.notifHour : 8;
      ...
      if (overdue.length > 0) {
        ...
        const daysLabel = first.days === 1 ? "tomorrow" : "in " + first.days + " days";
        title = "Beekind Buzz \u2013 Upcoming tasks";
        body  = first.tasks.length + " task" + (first.tasks.length > 1 ? "s" : "") + " due " + daysLabel + ": " +
          first.tasks.slice(0, 2).map(d => d.data().title).join(", ") +
          (first.tasks.length > 2 ? " and more" : "");
      }
